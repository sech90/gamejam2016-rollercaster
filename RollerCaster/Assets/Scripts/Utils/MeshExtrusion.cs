...
public class MeshExtrusion
{
	...
	public static Vector2[] GetBoundary(Mesh m, int verticesNum){
		Edge[] edges = BuildManifoldEdges(m);

		List<Vector2> vertices = new List<Vector2>();
		Vector3[] allVertex = m.vertices;

		int offset = edges.Length/verticesNum;

//		float min_dist = 100000;
		Vector2 prev = allVertex[edges[0].vertexIndex[0]];
		...
		for(int i=1; i<edges.Length; i++){
			...
			curr = allVertex[edges[i].vertexIndex[0]];
			curr.y = allVertex[edges[i].vertexIndex[0]].z;

			if(Vector2.Distance(prev,curr) > 0.25f){
				vertices.Add(curr);	
				prev = curr;
				i+=offset-1;
			}
		}
	//	vertices.Add(allVertex[edges[0].vertexIndex[0]]);
